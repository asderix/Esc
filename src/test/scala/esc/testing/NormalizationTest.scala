...
/**
 ...
 class NormalizationTest extends FunSuite {
   ...
   test("Normalization.PersonName.2") {
       val normalizer = new NameNormalizer    
     assert(normalizer.normalizePersonName("Hans-Peter Meyer").normNames.toString == "Vector(Vector((hanspeter,1.0,1), (meyer,1.0,1)), Vector((hans,0.6,1), (peter,0.09999999999999998,1), (meyer,1.0,1)))")
   }
   ...
   test("Normalization.PersonName.4") {
       val normalizer = new NameNormalizer    
     assert(normalizer.normalizePersonName("Van de Saar Nikolowitsch").normNames.toString == "Vector(Vector((vandesaar,1.0,1), (nikolovic,1.0,1)), Vector((vande,0.19999999999999996,1), (saarnikolovic,1.0,1)), Vector((vande,0.19999999999999996,1), (saar,1.0,1), (nikolovic,1.0,1)))")
   }

   test("Normalization.OrgName.1") {
       val normalizer = new NameNormalizer    
     assert(normalizer.normalizeOrganisationName("Huber Office (Schweiz) AG").normNames.toString == "Vector(Vector((huberoffice,1.0,1), (ch,0.5,3), (ag,0.25,2)), Vector((huber,1.0,1), (office,0.5,1), (ag,0.25,2)), Vector((huber,1.0,1), (office,0.5,1), (ch,0.5,3)), Vector((huber,1.0,1), (office,0.5,1), (ch,0.5,3), (ag,0.25,2)))")
   }
   test("Normalization.OrgName.2") {
       val normalizer = new NameNormalizer    
     assert(normalizer.normalizeOrganisationName("Huber-Müller - - Meier AG").normNames.toString == "Vector(Vector((hubermuller,1.0,1), (meier,0.5,1), (ag,0.25,2)), Vector((huber,1.0,1), (mullermeier,0.5,1), (ag,0.25,2)), Vector((huber,1.0,1), (muller,0.3333333333333333,1), (meier,0.3333333333333333,1), (ag,0.25,2)))")
   }
   // -- Dates -- //
