...
class NameNormalizer(
  ...
  /** This method normalize a person name.
    *
    * @param fullName
    *   String representing the full name of a person, including middle- und
    *   maiden name(s) etc. Without title.
    *
    * @return
  ...
  def normalizePersonName(fullName: String): NormalizedName = {
    ...
    val mutWhitespaceVariations =
      mutable.ArrayBuffer.empty[mutable.ArrayBuffer[String]]
    val whitespaceSplit = TextNormalizer
      .normalize(fullName)
      .split(" ")
    ...
    whitespaceSplit.length match {
      ...
      case _ =>
        mutWhitespaceVariations += whitespaceSplit.to(mutable.ArrayBuffer)
    }

    val whitespaceVariations = mutWhitespaceVariations.map(_.toVector).toVector
    val mutNameElementCombinations =
      mutable.ArrayBuffer.empty[mutable.ArrayBuffer[(String, Double, Byte)]]

    whitespaceVariations.foreach(e => {
      buildPersonNameElementCombinations(e, mutNameElementCombinations)
    })

    val nameElementCombinations =
      mutNameElementCombinations.map(_.toVector).toVector
  ...
  def normalizeOrganisationName(fullName: String): NormalizedName = {
    ...
    val mutWhitespaceVariations =
      mutable.ArrayBuffer.empty[mutable.ArrayBuffer[String]]
    val whitespaceSplit = TextNormalizer
      .normalizeWithLegalForm(fullName)
      .split(" ")
      .map(e => TextNormalizer.normalizeNameElement(e))
      .toVector
  ...
  /** Return if a name element is reduced (true/false) and the reduced value.
    ...
    */
  def persNameElementReducedWeight(
      nameElement: String,
    ...
    nameElement match {
      ...
      case _ => {
        nameElement.length match {
          case 1 =>
            if (similarityConfig.allowOneLetterAbbreviation)
              reducedWeight =
                initialWeight - similarityConfig.oneLetterAbbreviationWeight;
            isReduced = true
  ...
  // ---
  private def buildWhitespaceVariations(
        ...
        (
          ...
          case _ => {
            (e._1.isCountry, whitespaceSplit(e._2 + 1).isCountry) match {
              ...
              case _ =>
                whitespaceCombination += e._1 + whitespaceSplit(e._2 + 1)
            }
          }
        }
      }
  ...
  private def buildPersonNameElementCombinations(
      ...
      mutNameElementCombinations: mutable.ArrayBuffer[
        mutable.ArrayBuffer[(String, Double, Byte)]
      ]
  ): Unit = {

    val mutNameElementVariationsList =
      mutable.ListBuffer.empty[mutable.ListBuffer[Vector[String]]]
    ...
    val nameElementVariationsList =
      mutNameElementVariationsList.map(_.toList).toList
      ...
      cl.foreach(e => {
        e.length match {
          case 1 =>
            mutNameElement += ((
              e(0),
              persNameElementReducedWeight(e(0), 1.0)._1,
              1
            ))
          case _ =>
  ...
  // ---
  private def buildOrganisationNameElementCombinations(
      whitespaceVariations: Vector[String],
      mutNameElementCombinations: mutable.ArrayBuffer[
        mutable.ArrayBuffer[(String, Double, Byte)]
      ]
  ...
  // ---
  private def combineSubLists[T](list: List[List[T]]): List[List[T]] =
    list match {
      case Nil => Nil :: Nil
      case head :: tail =>
        val rec = combineSubLists[T](tail)
        rec.flatMap(r => head.map(t => t :: r))
    }

  // ---
  private def firstNameElementReducedWeight(
      nameElement: String,
      initialWeight: Double
  ): Double = {
    var reducedWeight: Double = 1.0
    nameElement match {
      ...
      case _ =>
        reducedWeight =
          persNameElementReducedWeight(nameElement, initialWeight)._1
    }
  ...
  // ---
  private def orgNameElementReducedWeight(
      nameElement: String,
      initialWeight: Double
  ): Double = {
    var reducedWeight: Double = 1.0
  ...
  // ---
  private def getOrgNameElementTypeWeightNorm(
      nameElement: String,
      initWeight: Double
  ): (Byte, Double, String) = {
    nameElement.toDeLegalForm match {
      case (true, x: String) =>
        return (2, similarityConfig.normOrgLegalformWeight, x)
      case (_, _) =>
    }

    nameElement.toIsoCountry match {
      case (true, x: String) =>
        return (3, similarityConfig.normOrgCountryWeight, x)
      case (_, _) =>
    }
