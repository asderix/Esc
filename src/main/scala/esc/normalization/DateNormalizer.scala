...
object DateNormalizer {
  ...
  /** Returns a NormalizedDate object for a given string. The following date
    ...
    *   - yyyy = 2019 All variants with dots (.) are also supported with hyphen
  ...
  def normalizeDate(dateString: String): NormalizedDate = {
    dateString match {
      case dateString if BasicFunctions.isNullOrEmpty(dateString) =>
        return NormalizedDate(0, 0, 0, 99)
      case _ =>
    }
    ...
    mutDateString match {
      case yyyyMMdds(_*) =>
        try {
          val fromCustomPattern =
            LocalDate.parse(mutDateString, DateTimeFormatter.BASIC_ISO_DATE)
          return NormalizedDate(
            fromCustomPattern.getYear(),
            fromCustomPattern.getMonth().getValue(),
            fromCustomPattern.getDayOfMonth(),
            0
          )
        } catch {
      ...
      case mMMMdyyyy(_*) =>
        try {
          val fromCustomPattern = LocalDate.parse(
            mutDateString,
      ...
      case yyyyMMd(_*) =>
        try {
          val fromCustomPattern = LocalDate.parse(
            mutDateString,
            DateTimeFormatter.ofPattern("yyyy.MM.d", Locale.ENGLISH)
          )
          return NormalizedDate(
            fromCustomPattern.getYear(),
            fromCustomPattern.getMonth().getValue(),
      ...
      case yyyyMdd(_*) =>
        try {
          val fromCustomPattern = LocalDate.parse(
            mutDateString,
            DateTimeFormatter.ofPattern("yyyy.M.dd", Locale.ENGLISH)
          )
          return NormalizedDate(
            fromCustomPattern.getYear(),
            fromCustomPattern.getMonth().getValue(),
      ...
      case yyyyMM(_*) =>
        ...
        na(1).toInt match {
          case x if (x < 13 && x > 0) =>
            return NormalizedDate(na(0).toInt, x, 0, 1)
