...
/**
  ...
  */
class Indexer(val indexWriter : IndexWriter, val similarityConfig : SimilarityConfig = new SimilarityConfig()) {
    ...
    /**
      ...
      * If a person has several names (e.g. alternative spellings, abbreviations or artist names etc.),
      * the person must be added several times. You can add the same id several times. But this is not recommended!
      * You get problems when updating the person by id. Recommended solution: Add different ids (e.g. with
      * post-fix) and use the externalId as your unique id. You can configure (SimilarityConfig) whether
      * the best match is selected/grouped by id oder externalId.
      *
      * @param  person  The person object for adding to the index.
      */
    def addPerson(person : IndexPerson) : Boolean = {
      val normalizedName = nameNormalizer.normalizePersonName(person.fullName)
      
      indexWriter.addDocument(createDocument(person.id,
                                             person.externalId,
                                             person.fullName,
                                             normalizedName,
    ...
    /**
      ...
      * If an organisation has several names (e.g. alternative spellings, abbreviations etc.),
      * the organisation must be added several times. You can add the same id several times. But this is not recommended!
      * You get problems when updating the person by id. Recommended solution: Add different ids (e.g. with
      * post-fix) and use the externalId as your unique id. You can configure (SimilarityConfig) whether
      * the best match is selected/grouped by id oder externalId.
      *
      * @param  organisation  The organisation object for adding to the index.
      */
    def addOrganisation(organisation : IndexOrganisation) : Boolean = {
      val normalizedName = nameNormalizer.normalizeOrganisationName(organisation.fullName)
      
      indexWriter.addDocument(createDocument(organisation.id,
    ...
    /**
      * Update a person.
      *
      * @param  person  IndexPerson with the new values. The id is relevant for selection the person to update.
      */
    def updatePerson(person : IndexPerson) : Boolean = {
      val normalizedName = nameNormalizer.normalizePersonName(person.fullName)
      
      indexWriter.updateDocument(new Term("id", person.id), createDocument(person.id,
                                             person.externalId,
                                             person.fullName,
                                             normalizedName,
                                             normalizedName.normNames.toCompactJson,
    ...
    /**
      ...
      * 
      * @param  organisation  IndexOrganisation with the new values. The id is relevant for selection the person to update.
      */
    def updateOrganisation(organisation : IndexOrganisation) : Boolean = {
      val normalizedName = nameNormalizer.normalizeOrganisationName(organisation.fullName)
      
      indexWriter.updateDocument(new Term("id", organisation.id), createDocument(organisation.id,
                                             organisation.externalId,
                                             organisation.fullName,
                                             normalizedName,
                                             normalizedName.normNames.toCompactJson,
    ...
    //
    private def createDocument(id : String, externalId : String, fullName : String, normalizedName : NormalizedName, jsonName : String, nameType : String, 
      dates : List[String], countries : List[String], label : String) : Document = {      
          ...
          element._3 match {
            case 1 => {                            
              nameNormalizer.persNameElementReducedWeight(element._1, 1.0) match {
                case (_, false) => {
                  nameDoc += nameElementSimilarityDb.getMatchList(element._1, similarityConfig.nameElementSimilarityForHit).mkString(" ")
